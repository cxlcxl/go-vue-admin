<template>
  <el-dialog title="密码修改" :visible.sync="visible" append-to-body width="380px" :close-on-click-modal="false">
    <el-form ref="reset_pass" :model="form" label-width="100px" :rules="rules">
      <el-form-item label="原密码" prop="original_pass">
        <el-input v-model="form.original_pass" type="password" placeholder="请输入原密码" />
      </el-form-item>
...
export default {
  ...
  data() {
    var validatePass = (rule, value, callback) => {
      if (!validPass(value)) {
        callback(new Error('密码限字母开头 6-20 位的字符'))
      } else {
        callback()
      }
    }
    ...
    return {
      ...
      rules: {
        ...
        original_pass: [{ required: true, message: '原密码必填' }, { validator: validatePass, trigger: 'blur' }]
      }
    }
  },
  methods: {
